...
async def student_view(api: TrainingCenterApi) -> None:
    enrollments = await api.my_enrollments()
    taken = {e.course_id for e in enrollments if e.status == 'active'}
    print('My courses:', [e.course.title for e in enrollments if e.course] or 'none yet')
    cheapest = [c async for c in api.all_courses(sort='price') if c.id not in taken]
    if cheapest:
        enrollment = await api.enroll(cheapest[0].id)
        print(f'Enrolled in the cheapest free course: {cheapest[0].title} ({enrollment.status})')
